    ...
    const oldestToYoung = inventors.sort((a, b) => a.year > b.year ? 1 : -1);
    console.table(oldestToYoung);
    // Array.prototype.reduce()
    // 4. How many years did all the inventors live all together?
    const totalYears = inventors.reduce(((total, {passed, year}) => total + (passed - year)), 0);
...
// console.log(de);
    ...
    const abcDirection = people.sort();
    console.table(abcDirection);
    // 8. Reduce Exercise
    // Sum up the instances of each of these
    const data = ['car', 'car', 'truck', 'truck', 'bike', 'walk', 'car', 'van', 'bike', 'walk', 'car', 'van', 'car', 'truck' ];
    const transport = data.reduce(function (element, index) {
            if (!element[index]) {
                element[index] = 0;
                }
            element[index]++;
